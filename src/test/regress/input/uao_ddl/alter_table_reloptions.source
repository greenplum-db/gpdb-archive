...
-- Check if the reloptions have been correctly changed from the defaults.
SELECT blocksize,compresslevel,checksum,compresstype
FROM pg_appendonly WHERE relid='aorelopts_@amname@'::regclass::oid;

-- Check pg_class.reloptions too.
...
-- pg_class.reloptions should show nothing.
SELECT reloptions FROM pg_class WHERE relname = 'aorelopts_@amname@';
